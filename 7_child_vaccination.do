...
if inlist(name,"Bangladesh2014"){
		...
		replace c_measles = 0 if missing(c_measles) & !missing(h1)		
		replace c_measles  = . if h9 > 3 | sm15 > 3	
}
...
if inlist(name,"Jordan2012"){
		...
		replace c_measles = 0 if missing(c_measles) & !missing(h1)		
		replace c_measles  = . if h9 > 3 | s506mm > 3
}	
